...
Ваша задача — покрыть тестами Java-приложение.
...
**Важно:** Отчёт ваших автотестов будет читать человек, не связанный с программированием.
...
Для доступа к эндпоинту требуется заголовок X-Api-Key. Это статический API-ключ, который проверяется приложением при каждом запросе.
...
У внешнего сервиса только два эндпоинта. На них отправляет запросы тестируемое приложение. Оба могут возвращать любое тело ответа, тестируемое приложение смотрит только на код ответа (HTTP status code).

Пример запроса на /auth:
